...
(function ($) {
	...
	var egg = [
		" \\___/|_____|___| |_____/_/   \\_|____/_/   \\_|_| \\_|",
		"| |_| | |___ | |  | |___ / ___ \\| |_) / ___ \\| |\\  |",
		"| | | | |    | |  | |     / _ \\ |  _ \\ / _ \\ |  \\| |",
		" / _ \\| |   |_ _| | |      / \\  | __ )  / \\  | \\ | |",
	...
	var secretString = "V2FudCB0byBoaXJlIG1lPyAKVXNlIHRoZSBjb250YWN0IGZvcm0gb24gbXkgaG9tZXBhZ2UgKGh0dHA6Ly9ibHVlZm9udC5kZXNpZ24vKSAKb3IgZW1haWwgbWUgYXQgb2xpQGJsdWVmb250LmRlc2lnbi4=";

	function easter(egg) {
		for (var i = egg.length - 1; i >= 0; i--) {
			console.log(egg[i]);
		}
		
		console.log(Base64.decode(secretString));
	}
...
(function ($) {
	...
	var $body,
		settings = {
			scrollDuration: 600 // in ms
		};

	/* open/close nav on hamburger click */
	...
	function onInternalLinkClick (e) {
		...
		if ($target.length > 0) {
			...
			// animate 'html, body' to simulate scroll
			$body.animate({
	...
	/* bind items to click listeners */
	function bindings () {
		$('.nav-toggle').on('click', onBtnClick);

		$('.scroll-link').on('click', onInternalLinkClick);	
	}

	/* assign var(s) and call bindings on document ready */
	$(document).ready(function () {
...
(function ($) {
	...
	var $window,
		...
		$sibling,
		bannerSettings = {
			transitionStart:    1,				// in px
			transitionEnd: 		400,			// in px
			colorStart: 		[253, 253, 253],
			colorEnd: 			[17, 17, 17]
		},
		bannerColorDiff = [
			bannerSettings.colorEnd[0] - bannerSettings.colorStart[0], 
			bannerSettings.colorEnd[1] - bannerSettings.colorStart[1], 
			bannerSettings.colorEnd[1] - bannerSettings.colorStart[0]];

	/* parallax & color transition animation on .name-banner */
	function animateNameBanner (e) {
		var $this = $(this),
			wScroll = $this.scrollTop(),
			$banner = $('.name-banner');

		var transitionPercent = (wScroll - bannerSettings.transitionStart) / 
				(bannerSettings.transitionEnd - bannerSettings.transitionStart);
		var roundedPercent = Math.min(1, Math.max(0, transitionPercent));
		var bgColor = [
        	Math.round(bannerSettings.colorStart[0] + bannerColorDiff[0] * roundedPercent), 
        	Math.round(bannerSettings.colorStart[1] + bannerColorDiff[1] * roundedPercent), 
        	Math.round(bannerSettings.colorStart[2] + bannerColorDiff[2] * roundedPercent)];

		$banner.css({
			'transform': 'translateY(' + wScroll / 3.5 + '%)',		// parallax
			'background-color': 'rgb(' + bgColor.join(',') + ')'	// color transition
		});
	}

	/* called on document ready to check if section is already visible
		if so, add .already-visible to prevent animation */
	function checkSections () {
		$sections.each(function(i, el) {
			var $section = $(el);

			if ($section.visible(true)) {
				$section.addClass("already-visible"); 
			} 
		});
	}

	/* add .come-in to each section when visible on the screen for CSS animation */
	...
	function scrollHandlers () {
		...
		// check if desktop layout is active before animating .name-banner
		if ($window.width() > 767) {
			$window.scroll(animateNameBanner);
		}

		$window.scroll(animateSections);
	}

	/* assign var(s) and call functions on document ready */
	$(document).ready(function() {
		
		$window = $(window);
		$sections = $('.slide-up'); // sections now slide horizontally, not vertically
...
(function ($) {
	...
	function validateField (e) {
		...
		switch (type) {
			case 'name':
				if (chars > 0) {
					$field.parent().addClass('input-valid');
					isNameValid = true;
				} else {
					$field.parent().removeClass('input-valid');
					isNameValid = false;
				}
				isSubmitActive();
				break;
	...
	/* activate submit button if form inputs are valid */
	function isSubmitActive () {
		if (isNameValid && isEmailValid && isMessageValid && $inputWhinnie.val().length == 0) {
	...
	function sendForm (e) {
		var $this = $(this),
			url = $this.attr('action'); // 'action' attribute from form
		...
		if (isFormValid) {
			...
			.fail(function() {
				console.log("error");
				$errorEl.html(errorMessage.fail); // show error message on failure
			})
			.always(function() {
				console.log("complete");
				$submitBtn.focus().blur(); // remove focus state from submit button
			});
		}
	...
	/* check for messages returned from contact.php */
	function formSent (data) {
		switch (data) {
			// on form sent successfully
			case 'success':
				formSuccess();
				break;
			// on input not filled
			case 'error-nofilled':
				$errorEl.html(errorMessage.nofilled);
				break;
			// on email not valid
			case 'error-email':
				$errorEl.html(errorMessage.email);
				break;
		}
	}
	...
	function formSuccess () {
		// animate success element into view
		$successEl.css({
			transform: 'translateX(0)',
			opacity: '1'
		});	

		// animate form out of view
		$form.css({
	...
	/* add input & keyup listeners to form inputs & submit listener on the form */
	function bindings () {

		$inputName.on('input keyup', { type: 'name' }, validateField);
		$inputEmail.on('input keyup', { type: 'email' }, validateField);
		$inputMessage.on('input keyup', { type: 'message' }, validateField);

		$form.submit(sendForm);

	}

	/* assign vars and call bindings on document ready */
	$(document).ready(function () {
		...
		$inputWhinnie = $('input#input-whinnie');
		$submitBtn = $('.form-submit');
		$errorEl = $('.form--error-message');
		$successEl = $('.message-sent');
...
/* For checking if element is visible */
(function ($) {
...
/* For code highlighting within blog posts */
